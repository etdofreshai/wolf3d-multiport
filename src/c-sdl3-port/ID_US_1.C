...
//			from DOS.
//
//	NOTE: This was a DOS hard error handler. On modern systems, we just
//	print the error and abort.
//
///////////////////////////////////////////////////////////////////////////
static void
USL_HardError(const char *errstr)
{
extern	void	ShutdownId(void);

	abortprogram = (char *)errstr;
	ShutdownId();
...
US_Print(char *s)
{
	char	c,*se;
	word	w,h;
	// SDL3 port: Use a local buffer so we never write to read-only string literals.
	char	buf[256];
	int		len;
...
US_CPrint(char *s)
{
	char	c,*se;
	// SDL3 port: Use a local buffer so we never write to read-only string literals.
	// Original DOS code wrote NUL terminators into the string to split on newlines,
	// but modern compilers put string literals in read-only memory.
	char	buf[256];
	int		len;
