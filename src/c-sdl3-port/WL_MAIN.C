// WL_MAIN.C
// Ported from DOS/Borland C to modern C with SDL3
...
// SDL3 port: standard argc/argv stored globally for compatibility with _argc/_argv pattern
int		_argc;
char	**_argv;
...
void Patch386 (void)
{
// SDL3 port: 386 detection and patching not needed on modern CPUs
	int     i;
	...
	// Modern CPUs are always at least 386-class
	IsA386 = true;
}
...
boolean SaveTheGame(FILE *file,int x,int y)
{
	...
	// SDL3 port: skip disk free space check - modern systems have plenty of space
...
void SignonScreen (void)                        // VGA version
{
	...
	// SDL3 port: signon screen memory reclamation removed
	// (no segmented memory model - introscn is linked data, not reclaimable)
}
...
#ifndef SPEAR
	VW_Bar (0,189,300,11,sdl_screenbuf[0]);  // SDL3: read pixel from framebuffer instead of peekb
	WindowX = 0;
	WindowW = 320;
	PrintY = 190;
...
#ifndef SPEAR
#ifndef UPLOAD
	// SDL3 port: replaced _dos_gettime with SDL_GetTicks
	start = (SDL_GetTicks() % 300) / 100 * 6;  // roughly same behavior: pick 0, 6, or 12
#else
	start = 0;
#endif
#else
...
void InitGame (void)
{
...
#endif
	{
		...
		ShutdownId();
		// SDL3 port: DOS text-mode movedata/gotoxy removed
		printf("Insufficient memory to run game.\n");
		exit(1);
	}
...
//
	...
	{
		NoWait = true;
		// SDL3 port: geninterrupt(0x60) removed - VR hardware interrupt not applicable
	}
}
...
void Quit (char *error)
{
	...
	if (error && *error)
	{
	  // SDL3 port: DOS text-mode display removed, just print error
	  printf("ERROR: %s\n", error);
	  exit(1);
	}
	else
	if (!error || !(*error))
	{
		// SDL3 port: DOS text-mode order screen display removed
	}
...
{
	// SDL3 port: store argc/argv in globals for Borland _argc/_argv compatibility
	_argc = argc;
	_argv = argv;
	...
	//
	// SDL3 port: _dos_getdate removed - beta date check disabled for modern builds
#endif
	...
	// SDL3 port: --nowait flag to skip input waits (for automated testing)
	// --autostart implies --nowait since there's no user to press keys.
	if (MS_CheckParm("nowait") || MS_CheckParm("autostart"))
		NoWait = true;

	// Frame capture flags
	if (MS_CheckParm("capture"))
	{
		capture_enabled = 1;
		SDL_CreateDirectory("captures");
	}

	// --capture-limit N : quit after N captured frames
	{
		int ci;
		for (ci = 1; ci < _argc; ci++)
		{
			char *p = _argv[ci];
			while (*p && !isalpha(*p)) p++;
			if (!SDL_strcasecmp(p, "capture-limit") || !SDL_strcasecmp(p, "capturelimit"))
			{
				if (ci + 1 < _argc)
					capture_limit = atoi(_argv[ci + 1]);
				break;
			}
		}
	}

	// --quit-after MS : auto-quit after MS milliseconds
	{
		int ci;
		for (ci = 1; ci < _argc; ci++)
		{
			char *p = _argv[ci];
			while (*p && !isalpha(*p)) p++;
			if (!SDL_strcasecmp(p, "quit-after") || !SDL_strcasecmp(p, "quitafter"))
			{
				if (ci + 1 < _argc)
					quit_after_ms = (Uint64)atoi(_argv[ci + 1]);
				break;
			}
		}
	}

	// --test-sequence : inject key events at specific frames for automated testing
	if (MS_CheckParm("test-sequence") || MS_CheckParm("testsequence"))
		test_sequence_enabled = 1;

	InitGame ();

	// --autostart flag: skip menus and start Episode 1, difficulty 1 (easy)
	if (MS_CheckParm("autostart"))
	{
		NewGame(1, 0);  // difficulty 1, episode 0
		startgame = true;
		GameLoop();
		Quit(NULL);
	}
